...
const CalciteBlock = class {
  ...
  renderScrim() {
    ...
    return [loading || disabled ? index.h("calcite-scrim", { loading: loading }) : null, defaultSlot];
  }
  renderIcon() {
    var _a;
    const { el, status } = this;
    const icon = (_a = ICONS$1[status]) !== null && _a !== void 0 ? _a : false;
    const hasIcon = dom.getSlotted(el, SLOTS.icon) || icon;
    const iconEl = !icon ? (index.h("slot", { name: SLOTS.icon })) : (index.h("calcite-icon", { class: {
  ...
  render() {
    ...
    const headerContent = (index.h("header", { class: CSS$1.header }, this.renderIcon(), index.h("div", { class: CSS$1.title }, index.h(CalciteHeading.CalciteHeading, { class: CSS$1.heading, level: headingLevel || HEADING_LEVEL }, heading), summary ? index.h("div", { class: CSS$1.summary }, summary) : null)));
    ...
    const headerNode = (index.h("div", { class: CSS$1.headerContainer }, this.dragHandle ? index.h("calcite-handle", null) : null, collapsible ? (index.h("button", { "aria-expanded": collapsible ? open.toString() : null, "aria-label": toggleLabel, class: CSS$1.toggle, onClick: this.onHeaderClick, title: toggleLabel }, headerContent, !hasControl ? (index.h("calcite-icon", { "aria-hidden": "true", class: CSS$1.toggleIcon, icon: collapseIcon, scale: "s" })) : null)) : (headerContent), loading ? (index.h("calcite-loader", { inline: true, "is-active": true, label: intlLoading })) : hasControl ? (index.h("div", { class: CSS$1.controlContainer }, index.h("slot", { name: SLOTS.control }))) : null, hasMenuActions ? (index.h("calcite-action-menu", { label: this.intlOptions || TEXT$1.options }, index.h("slot", { name: SLOTS.headerMenuActions }))) : null));
...
const CalciteBlockSection = class {
  ...
  render() {
    ...
    const headerNode = toggleDisplay === "switch" ? (index.h("label", { "aria-label": toggleLabel, class: {
      ...
      }, onKeyDown: this.handleHeaderLabelKeyDown, tabIndex: 0, title: toggleLabel }, index.h("div", { class: CSS.toggleSwitchContent }, index.h("span", { class: CSS.toggleSwitchText }, text)), index.h("calcite-switch", { onCalciteSwitchChange: this.toggleSection, scale: "s", switched: open, tabIndex: -1 }), this.renderStatusIcon())) : (index.h("button", { "aria-label": toggleLabel, class: {
        [CSS.sectionHeader]: true,
        [CSS.toggle]: true
      }, name: toggleLabel, onClick: this.toggleSection, onKeyDown: this.handleHeaderLabelKeyDown }, index.h("calcite-icon", { icon: arrowIcon, scale: "s" }), index.h("span", { class: CSS.sectionHeaderText }, text), this.renderStatusIcon()));
    return (index.h("section", { "aria-expanded": open.toString(), class: { [dom.CSS_UTILITY.rtl]: dir === "rtl" } }, headerNode, index.h("div", { class: CSS.content, hidden: !open }, index.h("slot", null))));
  }
  get el() { return index.getElement(this); }
};
