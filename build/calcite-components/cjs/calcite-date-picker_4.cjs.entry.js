...
const CalciteDatePicker = class {
  constructor(hostRef) {
    ...
    this.monthHoverChange = (e) => {
      ...
      if (!this.proximitySelectionDisabled) {
        if (this.endAsDate) {
          const startDiff = calciteDatePickerResources.getDaysDiff(date, this.startAsDate);
          const endDiff = calciteDatePickerResources.getDaysDiff(date, this.endAsDate);
    ...
    this.monthDateChange = (e) => {
      const date = new Date(e.detail);
      if (!this.range) {
        this.value = calciteDatePickerResources.dateToISO(date);
        this.activeDate = date;
        return;
      }
      if (!this.startAsDate || (!this.endAsDate && date < this.startAsDate)) {
        if (this.startAsDate) {
          ...
          this.end = calciteDatePickerResources.dateToISO(newEndDate);
          this.setEndAsDate(newEndDate, true);
          this.activeEndDate = newEndDate;
        }
        this.start = calciteDatePickerResources.dateToISO(date);
        this.setStartAsDate(date, true);
        this.activeStartDate = date;
      }
      else if (!this.endAsDate) {
        this.end = calciteDatePickerResources.dateToISO(date);
  ...
  render() {
    ...
    return (index.h(index.Host, { onBlur: this.reset, onKeyUp: this.keyUpHandler, role: "application" }, this.renderCalendar(activeDate, dir, maxDate, minDate, date, endDate)));
  }
  valueWatcher(value) {
  ...
  getActiveDate(value, min, max) {
    return calciteDatePickerResources.dateFromRange(this.activeDate, min, max) || value || calciteDatePickerResources.dateFromRange(new Date(), min, max);
  }
  getActiveStartDate(value, min, max) {
    return (calciteDatePickerResources.dateFromRange(this.activeStartDate, min, max) || value || calciteDatePickerResources.dateFromRange(new Date(), min, max));
  }
  getActiveEndDate(value, min, max) {
    return (calciteDatePickerResources.dateFromRange(this.activeEndDate, min, max) || value || calciteDatePickerResources.dateFromRange(new Date(), min, max));
  }
...
const CalciteDatePickerMonth = class {
  ...
  render() {
    ...
    const days = [
      ...
      ...curMonDays.map((day) => {
        ...
        const active = calciteDatePickerResources.sameDate(date, this.activeDate);
  ...
  addMonths(step) {
    ...
    this.calciteDatePickerActiveDateChange.emit(calciteDatePickerResources.dateFromRange(nextDate, this.min, this.max));
   ...
   */
  isSelected(date) {
    return (calciteDatePickerResources.sameDate(date, this.selectedDate) ||
      (this.startDate && calciteDatePickerResources.sameDate(date, this.startDate)) ||
      (this.endDate && calciteDatePickerResources.sameDate(date, this.endDate)));
  }
  /**
  ...
  isStartOfRange(date) {
    return (!!this.startDate &&
      !calciteDatePickerResources.sameDate(this.startDate, this.endDate) &&
      calciteDatePickerResources.sameDate(this.startDate, date) &&
      !this.isEndOfRange(date));
  }
  isEndOfRange(date) {
    return ((!!this.endDate && !calciteDatePickerResources.sameDate(this.startDate, this.endDate) && calciteDatePickerResources.sameDate(this.endDate, date)) ||
      (!this.endDate &&
        this.hoverRange &&
        calciteDatePickerResources.sameDate(this.startDate, this.hoverRange.end) &&
        calciteDatePickerResources.sameDate(date, this.hoverRange.end)));
  }
  /**
  ...
  renderDateDay(active, day, dir, date, currentMonth, ref) {
    ...
    const isHoverInRange = this.isHoverInRange() ||
      (!this.endDate && this.hoverRange && calciteDatePickerResources.sameDate((_a = this.hoverRange) === null || _a === void 0 ? void 0 : _a.end, this.startDate));
    return (index.h("calcite-date-picker-day", { active: active, class: {
        "hover--inside-range": this.startDate && isHoverInRange,
        "hover--outside-range": this.startDate && !isHoverInRange,
        "focused--start": isFocusedOnStart,
        "focused--end": !isFocusedOnStart
      }, currentMonth: currentMonth, day: day, dir: dir, disabled: !calciteDatePickerResources.inRange(date, this.min, this.max), endOfRange: this.isEndOfRange(date), highlighted: this.betweenSelectedRange(date), key: date.toDateString(), localeData: this.localeData, onCalciteDayHover: this.dayHover, onCalciteDaySelect: this.daySelect, range: !!this.startDate && !!this.endDate && !calciteDatePickerResources.sameDate(this.startDate, this.endDate), rangeHover: this.isRangeHover(date), ref: (el) => {
  ...
  isRangeHover(date) {
    ...
    const cond1 = insideRange &&
      ((!isStart && date > this.startDate && (date < end || calciteDatePickerResources.sameDate(date, end))) ||
        (isStart && date < this.endDate && (date > start || calciteDatePickerResources.sameDate(date, start))));
    const cond2 = !insideRange &&
      ((!isStart && date >= this.endDate && (date < end || calciteDatePickerResources.sameDate(date, end))) ||
        (isStart &&
          (date < this.startDate || (this.endDate && calciteDatePickerResources.sameDate(date, this.startDate))) &&
          (date > start || calciteDatePickerResources.sameDate(date, start))));
    return cond1 || cond2;
  }
  get el() { return index.getElement(this); }
};
...
const CalciteDatePickerMonthHeader = class {
  ...
  renderContent(dir) {
    ...
    const localizedYear = calciteDatePickerResources.localizeNumber(this.activeDate.getFullYear(), this.localeData);
    const iconScale = this.scale === "l" ? "m" : "s";
    const order = calciteDatePickerResources.getOrder(unitOrder);
  ...
  setNextPrevMonthDates() {
    ...
    this.nextMonthDate = calciteDatePickerResources.dateFromRange(calciteDatePickerResources.nextMonth(this.activeDate), this.min, this.max);
    this.prevMonthDate = calciteDatePickerResources.dateFromRange(calciteDatePickerResources.prevMonth(this.activeDate), this.min, this.max);
  }
  /**
  ...
  setYear(localizedYear, increment = 0) {
    ...
    const parsedYear = calciteDatePickerResources.parseNumber(localizedYear, localeData);
    ...
    if (year && inRange && length === localizedYear.length) {
      ...
      const inRangeDate = calciteDatePickerResources.dateFromRange(nextDate, min, max);
      this.calciteDatePickerSelect.emit(inRangeDate);
      yearInput.value = calciteDatePickerResources.localizeNumber(inRangeDate.getFullYear(), localeData);
    }
    else {
      // leave the current active date and clean up garbage input
      yearInput.value = calciteDatePickerResources.localizeNumber(activeDate.getFullYear(), localeData);
    }
  }
  get el() { return index.getElement(this); }
  static get watchers() { return {
